...
class config:
    ...
    """
    DEFAULTS = {'CLI': {'single_file_display_line': 10, 'compare_file_display_line': 5}, 'GUI': {'window_size': '1000x700', 'graph_max_words': 10, 'graph_figsize': [5.0, 4.0], 'analyze_max_words': 5, 'graph_bar_color_single': 'skyblue', 'graph_bar_color_compare1': 'skyblue', 'graph_bar_color_compare2': 'lightgreen', 'graph_title_fontsize': 12.0, 'graph_label_fontsize': 10.0, 'dark_mode': False, 'text_font_size': 10}}
    try:
...
def clean_text(text):
    ...
    if text is None:
        return ''
    cleaned_text = ''.join((char if 'a' <= char <= 'z' or '0' <= char <= '9' or char == ' ' or (char == '-') else ' ' for char in text.lower()))
...
def calculate_similarity(word_count1, word_count2):
    ...
    for freq2 in word_count2[1]:
        count2_freq += freq2
    return (common_freq / count1_freq * 100 if count1_freq > 0 else 0, common_freq / count2_freq * 100 if count2_freq > 0 else 0)
...
class GUI_APP:
    ...
    DARK_THEME = ['#1e1e1e', '#ffffff', '#2d2d2d', '#ffffff', '#3d3d3d', '#0078d7', '#ffffff', '#2d2d2d', '#1e1e1e', '#1e1e1e', '#2d2d2d']
    ...
    def __init__(self, root, size):
        ...
        self.theme_toggle = ttk.Checkbutton(self.theme_frame, text='Dark Mode', variable=self.theme_var, command=self.toggle_theme)
    ...
    def apply_theme(self):
        ...
        def _update_widget_colors(widget, theme):
            ...
            if widget_class == 'Text':
                widget.configure(background=theme[2], foreground=theme[3], insertbackground=theme[1], selectbackground=theme[5], selectforeground=theme[6], font=('Arial', config.text_font_size))
            elif widget_class == 'Listbox':
                widget.configure(background=theme[2], foreground=theme[3], selectbackground=theme[5], selectforeground=theme[6], font=('Arial', config.text_font_size))
        ...
        self.style.configure('TLabelframe.Label', background=theme[9], foreground=theme[1], font=('Arial', config.text_font_size))
        self.style.configure('TNotebook', background=theme[0])
        self.style.map('TNotebook.Tab', background=[('selected', theme[5]), ('!selected', theme[10])], foreground=[('selected', theme[6]), ('!selected', theme[1])])
        self.style.configure('TButton', background=theme[4], foreground=theme[1], font=('Arial', config.text_font_size))
        self.style.map('TButton', background=[('active', theme[5])], foreground=[('active', theme[6])])
        self.style.configure('TEntry', fieldbackground=theme[2], foreground=theme[3], insertcolor=theme[1], font=('Arial', config.text_font_size))
        self.style.configure('TCheckbutton', background=theme[8], foreground=theme[1], selectcolor=theme[2])
        self.style.map('TCheckbutton', background=[('active', theme[8])], foreground=[('active', theme[1])])
        self.style.configure('TLabel', background=theme[8], foreground=theme[1], font=('Arial', config.text_font_size))
    ...
    def create_compare_tab(self):
        ...
        compare_btn = ttk.Button(buttons_frame, text='Compare Files', command=self.compare_files)
        compare_btn.pack(side=tk.TOP, pady=10)
        self.compare_nltk = tk.BooleanVar(value=False)
        use_nltk = ttk.Checkbutton(buttons_frame, text='Use cosine similarity', variable=self.compare_nltk, command=update_file_labels)
        ...
        self.file1_stats_text = tk.Text(self.file1_stats_frame, height=5, width=40, wrap=tk.WORD)
        ...
        self.file2_stats_text = tk.Text(self.file2_stats_frame, height=5, width=40, wrap=tk.WORD)
    ...
    def create_search_tab(self):
        ...
        regex_check = ttk.Checkbutton(search_frame, text='Use Regex', variable=self.regex_var)
    ...
    def create_replace_tab(self):
        ...
        regex_check = ttk.Checkbutton(word_frame, text='Use Regex', variable=self.replace_regex_var)
        regex_check.pack(side=tk.LEFT, padx=5)
        replacement_frame = ttk.Frame(replace_frame)
        replacement_frame.pack(fill=tk.X, pady=5)
        ttk.Label(replacement_frame, text='Replacement text:').pack(side=tk.LEFT, padx=5)
    ...
    def create_config_tab(self):
        ...
        ttk.Label(single_file_frame, text='Number of words to display in Analyze File tab:').pack(side=tk.LEFT, padx=5)
        self.single_file_display_var = tk.StringVar(value=str(config.single_file_display_line))
        self.single_file_display_entry = ttk.Entry(single_file_frame, width=10, textvariable=self.single_file_display_var)
        ...
        ttk.Label(compare_file_frame, text='Number of words to display in Compare Files tab:').pack(side=tk.LEFT, padx=5)
        self.compare_file_display_var = tk.StringVar(value=str(config.compare_file_display_line))
        self.compare_file_display_entry = ttk.Entry(compare_file_frame, width=10, textvariable=self.compare_file_display_var)
        ...
        save_btn = ttk.Button(buttons_frame, text='Save settings', command=self.save_config)
        save_btn.pack(side=tk.RIGHT, padx=5)
        cancel_btn = ttk.Button(buttons_frame, text='Cancel changes', command=self.reset_last_save_config)
        cancel_btn.pack(side=tk.RIGHT, padx=5)
        reset_btn = ttk.Button(buttons_frame, text='Reset values to default', command=self.reset_default_config)
        ...
        info_text.insert(tk.END, 'These settings control how many words are displayed in the word lists and graphs. Changes will be saved to the configuration file and applied immediately once the save button is pressed.')
    ...
    def create_comparison_graph(self, word_count1, word_count2, canvas_frame_widget, canvas_widget, max_words=config.graph_max_words):
        ...
        width = 0.35
        ax.bar([i - width / 2 for i in x], counts1, width, label='File 1', color=config.graph_bar_color_compare1)
        ax.bar([i + width / 2 for i in x], counts2, width, label='File 2', color=config.graph_bar_color_compare2)
    ...
    def create_nltk_comparison_graph(self, word_count1, reference_word_counts, file1_name, reference_file_names, similarity_scores, canvas_frame_widget, canvas_widget, max_words=config.graph_max_words):
        """
        ...
        if not all_common_words:
            ...
            ax.text(0.5, 0.5, 'No common words found in top frequency lists', ha='center', va='center', fontsize=12)
            ax.set_xticks([])
            ax.set_yticks([])
            plt.tight_layout()
            ...
            canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
            return
        all_common_words_sorted = helpers.quick_sort(all_common_words, key=lambda word: word_count1[1][helpers.linear_search(word_count1[0], word)] if word in word_count1[0] else 0, reverse=True)
    ...
    def save_config(self):
        ...
        try:
            ...
            if helpers.any((x <= 0 for x in [single_file_display, compare_file_display, graph_max_words, analyze_max_words, graph_width, graph_height, title_fontsize, label_fontsize])):
...
def compare_files(file_path1, file_path2):
    ...
    if similarity > 80:
        ...
        print('\x1b[33mPlagiarism Level: MEDIUM - These texts have significant overlap\x1b[m')
    elif similarity > 20:
        print('\x1b[92mPlagiarism Level: LOW - These texts have some common elements\x1b[m')
    else:
        print('\x1b[32mPlagiarism Level: MINIMAL - These texts are mostly different\x1b[m')
